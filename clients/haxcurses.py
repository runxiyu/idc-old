...
def update_screen(stdscr, net_scr, user_scr):
    ...
    if len(message_list) > 0:
        net_scr.erase()
        i = 0
        for line in message_list[message_index]["messages"][::-1]:
            offset = -(
                -len("<" + line["source"] + "> " + line["message"])
                // curses.COLS
            )
            i += offset
            if i >= curses.LINES:
                if offset > 1 and i + 1 - offset < curses.LINES:
                    tmp = curses.LINES + offset - i - 1
                    net_scr.addstr(0, 0, str(tmp))
                break
            net_scr.addstr(
                curses.LINES - i - 1,
                0,
    ...
    while True:
        ...
        if len(input) > 1:
            if input == "KEY_DOWN":
                ...
                input_index = max(
                    0, min(len(input_str), input_index - 1)
                )
                user_scr.move(0, prompt_len + input_index)
                user_scr.refresh()
                ...
                if input_index > 0:
                    input_str = (
                        input_str[: input_index - 1]
                        + input_str[input_index:]
                    )
                    input_index -= 1
                    user_scr.erase()
                    user_scr.addstr(0, 0, prompt + input_str)
            ...
            if input.startswith("/"):
                ...
                if command == "query":
                    if not any(
                        e["username"] == args[0] for e in message_list
                    ):
                        try:
                            message_list.append(
                                {
                ...
                send(
                    s,
                    "PRIVMSG\tTARGET="
                    + message_list[message_index]["username"]
                    + "\tMESSAGE="
                    + input.replace("\\", "\\\\"),
                )
...
def main(stdscr):
    ...
    while True:
        ...
        if len(input) > 1:
            ...
            if not all(c[1] != "" for c in config):
                err_scr.erase()
                err_scr.addstr(
                    0, 0, "Not all required settings have been filled."
                )
                err_scr.refresh()
                config[line][0].move(0, config[line][2] + index)
                config[line][0].refresh()
    ...
    threading.Thread(
        ...
        args=(s, stdscr, net_scr, user_scr),
        daemon=True,
    ).start()
    ...
    while True:
        ...
        for line in lines:
            ...
            for x in [
                ...
                try:
                    args[x.split("=", 1)[0].upper()] = x.split("=", 1)[
                        1
                    ]
                except IndexError:
            ...
            if command == "PRIVMSG":
                if all(
                    args.get(x) != None
                    for x in ["TARGET", "SOURCE", "MESSAGE"]
                ):
                    ...
                    if not any(
                        c["username"] == buffer for c in message_list
                    ):
                        message_list.append(
                            {
                                "username": buffer,
