...
#define IDC_CHANNEL_MAX   200
#define IDC_MSG_MAX       65535 /* guaranteed to be <= than PIPE_BUF */
...
struct Channel {
	int fdin;
	char name[IDC_CHANNEL_MAX]; /* channel name (normalized) */
...
static char     idcpath[PATH_MAX]; /* idc dir (-i) */
static char     msg[IDC_MSG_MAX];  /* message buf used for communication */

static void
usage(void)
{
	fprintf(stderr, "usage: %s <-s host> [-i <idc dir>] [-p <port>] "
...
error:
	fprintf(stderr, "%s: path to idc directory too long\n", argv0);
	exit(1);
}
...
channel_new(const char *name)
{
	...
	create_filepath(c->inpath, sizeof(c->inpath), idcpath,
	                channelpath, "in");
	create_filepath(c->outpath, sizeof(c->outpath), idcpath,
	                channelpath, "out");
	return c;
}
...
static void
loginkey(int idcfd, const char *key)
{
...
static void
loginuser(int idcfd, const char *host, const char *fullname)
{
...
tokenize(char **result, size_t reslen, char *str, int delim)
{
	...
	while (*n != '\0') {
        ...
        // numeric replies don't exist though
...
static void
proc_channels_privmsg(int idcfd, Channel *c, char *buf)
{
	snprintf(msg, sizeof(msg), "<%s> %s", nick, buf);
	channel_print(c, msg);
	snprintf(msg, sizeof(msg), "PRIVMSG\tTARGET=%s\tMESSAGE=%s\r\n", c->name, buf);
	ewritestr(idcfd, msg);
}

static void
proc_channels_input(int idcfd, Channel *c, char *buf)
{
	char *p = NULL;
	size_t buflen;
	...
	if (buf[0] == '\0')
		return;
        /* TODO so we're having issues with null bytes */
	if (buf[0] != '/') {
		proc_channels_privmsg(idcfd, c, buf);
		return;
	}
...
// 					proc_channels_privmsg(idcfd, c, p + 1);
// 				return;
// 			}
// 			break;
// 		case 't': /* topic */
...
// 			return;
// 			break;
// lol all of the above commands aren't currently useful in IDC
...
static void
handle_channels_input(int idcfd, Channel *c)
{
	/*
	...
	proc_channels_input(idcfd, c, buf);
}

static void
handle_server_output(int idcfd)
{
	char buf[IDC_MSG_MAX];

	if (read_line(idcfd, buf, sizeof(buf)) == -1) {
...
static void
run(int idcfd, const char *host)
{
	Channel *c, *tmp;
	fd_set rdset;
	struct timeval tv;
	char ping_msg[IDC_MSG_MAX];
	int r, maxfd;
	...
	while (isrunning) {
		...
		if (r < 0) {
			...
			ewritestr(idcfd, ping_msg);
			continue;
		}
		if (FD_ISSET(idcfd, &rdset)) {
			handle_server_output(idcfd);
			last_response = time(NULL);
		}
		for (c = channels; c; c = tmp) {
			tmp = c->next;
			if (FD_ISSET(c->fdin, &rdset))
				handle_channels_input(idcfd, c);
		}
	}
}
...
main(int argc, char *argv[])
{
	...
	snprintf(prefix, sizeof(prefix), "%s/idc", spw->pw_dir);
	...
	r = snprintf(idcpath, sizeof(idcpath), "%s/%s", prefix, host);
	if (r < 0 || (size_t)r >= sizeof(idcpath)) {
		fprintf(stderr, "%s: path to idc directory too long\n", argv0);
		exit(1);
	}
	create_dirtree(idcpath);

	channelmaster = channel_add(""); /* master channel */
	if (key)
		loginkey(idcfd, key);
	loginuser(idcfd, host, fullname && *fullname ? fullname : nick);
	setup();
	run(idcfd, host);
	if (channelmaster)
